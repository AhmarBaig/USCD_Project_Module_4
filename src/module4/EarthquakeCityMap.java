...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(color(255, 150, 10));
		triangle(60, 95, 55, 104, 65, 104);
		...
		fill(255, 255, 0);
		ellipse(60, 200, 13, 13);
		fill(0,0,0);
		text("Shallow", 75, 200);
		
		fill(0, 0, 255);
		ellipse(60, 220, 13, 13);
		fill(0,0,0);
		text("Intermediate", 75, 220);
		
		fill(255, 0, 0);
		ellipse(60, 240, 13, 13);
		fill(0,0,0);
		text("Deep", 75, 240);
...
//				System.out.println(m);
	...
	private void printQuakes() 
	{
		...
		// Calculate Earthquakes within a specific country
		for (Marker cm: countryMarkers) {
			
			for (Marker qm: quakeMarkers) {
				EarthquakeMarker em = (EarthquakeMarker)qm;
				String name1 = (String)cm.getProperty("name");
				String name2 = (String)em.getProperty("country");
				
				if (em.isOnLand() && name2 == name1) {
					numQuakes += 1;
				}
				
			}
			if (numQuakes > 0) {
				System.out.println(cm.getProperty("name") + ": " + str(numQuakes));
			}
			numQuakes = 0;
		}
		
		// Calculate Ocean Quakes
		for (Marker om: quakeMarkers) {
			EarthquakeMarker oem = (EarthquakeMarker)om;
			if (!oem.isOnLand()) {
				numOceanQuakes += 1;
			}
		}
		
		System.out.println("OCEAN QUAKES: " + str(numOceanQuakes));
